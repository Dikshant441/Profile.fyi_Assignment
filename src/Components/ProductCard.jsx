...
const ProductCard = ({ cartData, Product }) => {
  ...
  const handleAddItem = (Product) => {
    ...
    console.log("Adding item to cart:", Product); // Debugging line
    dispatch(addItem(Product));
    setIsAdded(true);
    setTimeout(() => setIsAdded(false), 2000); // Reset feedback after 2 seconds
  };
  ...
  return (
    <div className="relative bg-white border border-gray-300 rounded-lg shadow-md overflow-hidden flex flex-col">
      <div className="p-4 flex flex-col flex-grow">
        ...
        <h3 className="text-base font-semibold mb-2 line-clamp-2">{category}</h3>
        ...
        {isAdded && (
          <motion.div
            className="absolute top-2 right-2 bg-green-500 text-white px-3 py-1 rounded-lg shadow-md"
